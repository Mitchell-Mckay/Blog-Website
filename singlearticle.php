...
if (isset($_GET['postid'])) {
   $id = $_GET['postid'];
   $query4 = "UPDATE `posts` SET `view_count` = `view_count` + 1 WHERE `post_id` = '$id'";
   mysqli_query($conn, $query4) or die("Bad query");
} else {
...
// query 1
$query = "SELECT posts.post_id, posts.author_id, posts.title, posts.content, posts.postCreated, posts.postUpdated, posts.view_count, category.category, users.fname, users.sname 
            FROM `posts` 
...
while ($row = mysqli_fetch_array($result)) {
   ...
   $created = $row['postCreated'];
   $updated = $row['postUpdated'];
   $content2 = $row['content'];
}

//query 2
$query2 = "SELECT comments.post_id, comments.user_id, users.username, comments.content, comments.publishedAt, comments.publish_stat
 FROM `comments` JOIN users ON comments.user_id = users.user_id WHERE `comments`.`publish_stat` = 'Yes' AND `comments`.`post_id` = '$id'";
...
if (isset($_POST['post'])) {
   ...
   if (!empty($content)) {
      $query3 = "INSERT INTO `comments` (`comment_id`, `post_id`, `user_id`, `content`, `publish_stat`) VALUES (NULL, '$postid', '$user_id', '$content', 'Yes')";

      mysqli_query($conn, $query3) or die("Bad query");

      header("Location:singlearticle.php?postid=$postid");
   } else {
   }
} else {
}
...
<head>
   ...
   <!-- CK Editor -->
   <script src="ckeditor4/ckeditor-basic/ckeditor.js"></script>
            ...
            <div class="row">
               <div class="col-md-9 post-content" data-aos="fade-up">
                  ...
                  <div class="single-post">
                     <span class="aside-title"><?php echo "$fname $sname" . "<br><br>"; ?></span>
                     <div class="post-meta">
                        <span class="date"><?php echo $category2; ?></span>
                        <span class="mx-1">&bullet;</span> <span>Created: <?php echo substr($created, 0, 10) . " | " . "Updated: " .  substr($updated, 0, 10); ?></span>
                        <span class="mx-1">&bullet;</span> <span><?php echo $view_count; ?> Views</span>
                     </div>
                     <h1 class="mb-5">
                        <?php echo $title2; ?>
                     </h1>
                  ...
                  <div class="comments">
                     <h3 class="comment-title py-4">Comments</h3>
                     <?php
                     while ($row = mysqli_fetch_array($result2)) {
                        $publishedAt = $row['publishedAt'];
                        $pub = substr($publishedAt, 0, 10);
                        ...
                        echo "<div class='comment d-flex'>
                           <div class='flex-shrink-1 ms-2 ms-sm-3'>
                              <div class='comment-meta d-flex'>
                                 ...
                                 <span class='text-muted'>$pub</span>
                              </div>
                        ...
                        <div class="col-lg-12">
                           ...
                           <div class="row">
                              <div class="col-12 mb-3">
                                 <p>
                                    <textarea class="form-control" name="comment" id="comment" placeholder="Enter your comment here" cols="30" rows="10"></textarea>
                                    <script>
