...
$query2 = "SELECT posts.title, posts.content FROM `posts` WHERE `posts`.`post_id` = '$post_id2'";

$result2 = mysqli_query($conn, $query2) or die("Bad query");

while ($row2 = mysqli_fetch_array($result2)) {
    $previoustitle = $row2['title'];
    $previousdescription = $row2['content'];
}
...
if (isset($_POST['update'])) {
    ...
    $set_clause = "";
    if (!empty($title2)) {
        $set_clause .= "`title` = '$title2', ";
    }
    if (!empty($content)) {
        $set_clause .= "`content` = '$content', ";
    }
    if (empty($set_clause)) {
        ...
        $query2 = "UPDATE `posts` SET $set_clause WHERE `posts`.`post_id` = '$post_id2'";
                    ...
                    <div class="row justify-content-center mt-3">
                        <div class="col-lg-12">
                            <form method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                                <h2 class="comment-title">Edit Post</h2>
                                <div class="row">
                                    <?php if ($msg != '') { ?>
                                    ...
                                    <?php } ?>
                                    <input type="hidden" name="post_id" value="<?php echo $post_id2; ?>">
                                    <div class="col-lg-6 mb-3">
                                        <label for="title">Title</label>
                                        <input type="text" class="form-control" id="title" value='<?php echo $previoustitle; ?>' />
                                    </div>
                                    <div class="col-12 mb-3">
                                        <p>
                                            <textarea class="form-control" id="content" name="content" cols="30" rows="10"> <?php echo $previousdescription; ?> </textarea>
                                            <script>
