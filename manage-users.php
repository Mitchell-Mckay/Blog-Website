...
if (isset($_GET['editButton'])) {
    ...
    header("Location:edit-user-admin.php?user_id=$user_id");
} elseif (isset($_GET['Activate'])) {
    $user_id = $_GET['user_id'];

    $query2 = "UPDATE `users` SET `status` = 'Active' WHERE `users`.`user_id` = '$user_id'";

    mysqli_query($conn, $query2) or die('Bad query');
} elseif (isset($_GET['Deactivate'])) {
    $user_id = $_GET['user_id'];

    $query2 = "UPDATE `users` SET `status` = 'Inactive' WHERE `users`.`user_id` = '$user_id'";

    mysqli_query($conn, $query2) or die('Bad query');
}

$query3 = "SELECT users.user_id, users.fname, users.sname, users.email, users.dob, users.username, users.level, users.registerDate, users.lastLogin, users.status FROM `users`";
                ...
                <div class="row">
                    <div data-aos="fade-up">
                        ...
                        <form method="GET" action="<?php echo $_SERVER['PHP_SELF']; ?>">
                            <table id="users" width="100%" cellpadding="5px">
                                ...
                                <tbody>
                                    ...
                                    while ($row2 = mysqli_fetch_array($result2)) {
                                        ...
                                        $useridbutton = "<input type = 'radio' name = 'user_id' id = 'user_id' value = '$user_id'>";
                                    ?>
                                        <tr>
                                            <td> <?php echo $useridbutton . $user_id; ?> </td>
                                            ...
                                            <td> <?php echo $row2['registerDate']; ?> </td>
                            ...
                            <div class=" col-12">
                                ...
                                <input class="btn btn-primary" type="submit" name="Deactivate" value="DEACTIVATE" />
                            </div>
                        </form>
